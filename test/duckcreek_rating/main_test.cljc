(ns duckcreek_rating.main-test
  "Contract + behavioral test for the duckcreek_rating-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "duckcreek_rating-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'duckcreek_rating.main-test)]
